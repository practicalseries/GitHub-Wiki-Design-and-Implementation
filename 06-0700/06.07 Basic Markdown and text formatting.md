...
> [!NOTE]<!-- NOTE ALERT -->
> *Although both* `<u>` *and* `<ins>` *work in Wiki* `.md` *files, the* `<u>` *does not work in repository Markdown files (for example the* `README.md` *file). Consequently, for compatibility, I recommend using the* `<ins>` *in place of* `<u>`; *see* <a href="05-markdown,-github-markdown-and-html#56markdown-difference-between-files">section&nbsp;5.6</a>.
